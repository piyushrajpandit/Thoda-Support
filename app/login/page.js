...
const Login = () => {
  ...
  useEffect(() => {
    document.title = mode === 'login' ? "Login - Get Me A Capital" : "Sign Up - Get Me A Capital";
  ...
  const handleLoginSubmit = async (e) => {
    ...
    try {
      ...
      if (res?.error) {
        toast.error(res.error || "Login failed. Check your phone number or password.");
      } else {
    ...
    <>
      ...
      <div className='text-white py-10 container mx-auto px-4 min-h-[85vh] flex flex-col justify-center items-center'>
        ...
        <p className="text-gray-400 text-sm mb-6 text-center max-w-sm">
          No OTP needed! Login instantly with your Phone Number / Username and Password.
        </p>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl shadow-xl w-full max-w-md p-6">
          ...
          {mode === 'login' ? (
            ...
            <form onSubmit={handleSignupSubmit} className="space-y-4">
              <div>
                <label className="block text-xs font-medium text-gray-300 mb-1">
                  Phone Number <span className="text-gray-500">(Used for simple login)</span>
                </label>
                <input
                  type="tel"
                  name="phone"
              ...
              <div>
                <label className="block text-xs font-medium text-gray-300 mb-1">
                  Username <span className="text-gray-500">(Your public page link)</span>
                </label>
                <input
                  type="text"
                  name="username"
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full py-3 bg-emerald-600 hover:bg-emerald-500 text-white font-medium rounded-lg shadow-md transition disabled:opacity-50 text-sm"
              >
                {loading ? "Creating Account..." : "Create Account (No OTP)"}
              </button>
            </form>
          )}
          ...
          <div className="relative my-6">
            ...
            <div className="relative flex justify-center text-xs">
              <span className="bg-slate-900 px-3 text-gray-400">Or continue with</span>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-2 items-center">
            <button
              ...
              type="button"
              className="flex items-center justify-center w-full bg-slate-50 text-black border border-gray-300 rounded-lg shadow-md py-2.5 text-sm font-medium hover:bg-gray-200 focus:outline-none transition"
            >
              <svg className="h-5 w-5 mr-2" viewBox="-0.5 0 48 48">
            ...
            <button
              ...
              type="button"
              className="flex items-center justify-center w-full bg-slate-800 text-white border border-slate-700 rounded-lg shadow-md py-2.5 text-sm font-medium hover:bg-slate-700 focus:outline-none transition"
            >
